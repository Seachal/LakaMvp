package com.laka.mvplibrary.view;
...
import com.laka.mvplibrary.observer.NetworkChangeObserver;
import com.laka.mvplibrary.presenter.IPresenter;
import com.laka.mvplibrary.utils.NetworkUtil;
import com.laka.mvplibrary.widget.dialog.CommonLoadingDialog;
...
public abstract class BaseActivity extends AppCompatActivity implements IView, NetworkChangeObserver.NetworkChangeListener {
    ...
    public void onNetworkChange(int state) {
        if (state == NetworkUtil.NETWORK_STATE_2G || state == NetworkUtil.NETWORK_STATE_3G
                || state == NetworkUtil.NETWORK_STATE_4G || state == NetworkUtil.NETWORK_STATE_UNKNOW
                || state == NetworkUtil.NETWORK_STATE_WIFI) {
            if (!mIsLostInternet) {
                return;
            }
            // 从无网络切换到有网络
